	...
	if n.Type == html.TextNode {
		...
		newText := wordRegex.ReplaceAllStringFunc(text, func(word string) string {
			if _, exists := misspelled[word]; exists {
				return `<span class='misspelled-word'>` + word + `</span>`
			}
			return word
		})
...
func ProcessHTML(htmlStr string, misspelled map[string][]string) (string, error) {
	...
	htmlStr = "<head><style>span.misspelled-word {text-decoration: underline; text-decoration-style: wavy; text-decoration-color: red;text-decoration-thickness: 1px;text-underline-offset: 3px;padding-left: 4px;padding-right: 4px;}</style></head>" + htmlStr
